...
- **EPC enrichment**: PPD comps can be enriched with EPC floor area via `enrich_epc=true` on the comps endpoint (or `--enrich-epc` in CLI). Groups comps by postcode, runs ONE summary search per distinct postcode, selects a unique candidate per comp (identity evidence only — an exact UPRN, or an exact address up to case, punctuation and a leading `Flat`/`Apartment` designator — refusing rather than guessing otherwise), then fetches ONLY that certificate — cached by certificate number. Attaches derived fields (`epc_floor_area_sqm`, `price_per_sqft`, `epc_rating`, etc.) plus the matched cert (`epc_match`), `epc_match_score` (always 100) and `epc_match_method` (`uprn` / `exact_address` / `address_designator_normalized`). Comps with no unique match are left un-enriched. After enrichment, call `compute_enriched_stats()` to populate `median_price_per_sqft` and `epc_match_rate`.
...
Create a `.env` file in the repo root (gitignored) with the variables you need.
...
Key variables:
- `EPC_API_TOKEN` - Required for EPC endpoints (GOV.UK Bearer API, England & Wales).
  `EPC_API_EMAIL` / `EPC_API_KEY` are deprecated: the service they authenticated against was retired.
